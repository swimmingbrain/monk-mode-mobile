import { View, Text, TouchableOpacity, AppState, Platform, Alert } from "react-native";
  ...
  const handleAppStateChange = (nextAppState: AppStateStatus) => {
    ...
    if (Platform.OS === 'ios') {
      if (nextAppState === 'inactive') {
        ...
        const transitionDuration = inactiveTimestamp.current ? Date.now() - inactiveTimestamp.current : -1;
        inactiveTimestamp.current = null; // Reset timestamp

        console.log(`iOS: App went background. Transition duration from inactive: ${transitionDuration}ms`);
        ...
        if (transitionDuration !== -1 && transitionDuration <= FOCUS_MODE_SCREEN_LOCK_THRESHOLD_MS) {
          ...
          console.log('iOS: Detected screen lock (fast inactive -> background).');
      ...
      console.log('Android: Timer/XP running status set to:', nextAppState === 'active');
    }
  };
